...
V_tplus1 = np.zeros((N, N, N, N, N))
V_t = np.zeros((N, N, N, N, N))
                    ...
                    for s5 in range(N):
                        ...
                        if t == T:
                            for a in range(num_actions):
                                rhs[a] = reward(s, a)
                            V_t[s1, s2, s3, s4, s5] = np.amax(rhs)
                        else:
                            for a in range(num_actions):
                                E = 0
                                        ...
                                        for A3_t in Ai_t_range:
                                            for A4_t in Ai_t_range:
                                                for A5_t in Ai_t_range:
                                                    ...
                                                    E += prob_Ai_t * V_tplus1[s_prime[0], s_prime[1], s_prime[2], s_prime[3], s_prime[4]]
                                rhs[a] = reward(s, a) + gamma*E
                            V_t[s1, s2, s3, s4, s5] = np.amax(rhs)
    V_tplus1 = np.copy(V_t)
    print(t)
            
plt.figure(figsize = (8, 6))
plt.plot(V_t[:,0,0,0,0])
plt.xlabel('$s_1$ = type 1 customers waiting', fontsize = 20)
plt.ylabel('V(t=0, $s_1$, $s_2$=0, $s_3$=0, $s_4$=0, $s_5$=0)', fontsize = 20)
plt.savefig('problem_2a-enumeration.png')
plt.close()
