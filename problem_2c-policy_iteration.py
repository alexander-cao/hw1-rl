...
pi_old = np.zeros((N, N, N, N, N))
pi_new = np.zeros((N, N, N, N, N))
                                    ...
                                    for A3_t in Ai_t_range:
                                        for A4_t in Ai_t_range:
                                            for A5_t in Ai_t_range:
                                                ...
                                                s_prime = new_state(s, A_t, pi_old[s1, s2, s3, s4, s5])
                                                E += prob_Ai_t * V_old[s_prime[0], s_prime[1], s_prime[2], s_prime[3], s_prime[4]]
                            V_new[s1, s2, s3, s4, s5] = reward(s, pi_old[s1, s2, s3, s4, s5]) + gamma*E

        V_norm = np.linalg.norm(np.ndarray.flatten(V_new - V_old), ord=np.inf)
        V_old = np.copy(V_new)
                                    ...
                                    for A3_t in Ai_t_range:
                                        for A4_t in Ai_t_range:
                                            for A5_t in Ai_t_range:
                                                ...
                                                E += prob_Ai_t * V_new[s_prime[0], s_prime[1], s_prime[2], s_prime[3], s_prime[4]]
                            rhs[a] = reward(s, a) + gamma*E
                        pi_new[s1, s2, s3, s4, s5] = np.argmax(rhs)

    pi_norm = np.linalg.norm(np.ndarray.flatten(pi_new - pi_old), ord=2)
...
plt.figure(figsize = (8, 6))
plt.plot(pi_new[:,0,0,0,0])
plt.xlabel('$s_1$ = type 1 customers waiting', fontsize = 20)
plt.ylabel('a = $\pi$($s_1$, $s_2$=0, $s_3$=0, $s_4$=0, $s_5$=0)', fontsize = 20)
plt.savefig('problem_2c-policy_iteration.png')
plt.close()
